...
const Reasons = () => {
  ...
  const handleAdd = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/master/add-reasons`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to add reasons');
      }

      setFormData(initialForm)
      fetchData()
      toast.success('Reasons added successfully!');
  ...
  const handleEdit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/master/edit-reasons`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${Token}`
        },
      ...
      const resData = await response.json();
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to edit reasons');
      }

      setFormData(initialForm)
      fetchData()
      toast.success('Reasons edited successfully!');
  ...
  const handleDelete = async (event) => {
    event.preventDefault();
    try {
      const response = await fetch(`${apiUrl}/master/delete-reasons/${departmentDetails.id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${Token}`
        }
      ...
      const resData = await response.json();
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to delete reasons');
      }
    ...
    <div>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Add New Reasons</h5>
              <button
                className="btn-close"
              ...
              <form onSubmit={handleAdd}>
                ...
                <div className="form-wrap">
                  <label className="col-form-label">
                    Reasons Name <span className="text-danger">*</span>
                  </label>
                  <input
                    type="text"
                    className="form-control"
                    name='name'
                    placeholder="Eg: ReasonsOne,ReasonsTwo"
                    value={formData.name}
                    required
                    onChange={handleInputChange}
                  />
                </div>
                <p style={{ fontSize: 'small', color: '#7695FF' }}>Note: You can add multiple Reasons at once by separating them with commas (,)</p>
      ...
      <div className="modal custom-modal fade" id="edit_source" role="dialog">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Edit Reasons</h5>
              <button
                className="btn-close"
                data-bs-dismiss="modal"
                aria-label="Close"
