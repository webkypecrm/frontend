...
const Source = () => {
  ...
  const handleAdd = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/master/add-source`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to add source');
      }
  ...
  const handleEdit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/master/edit-source`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${Token}`
        },
      ...
      const resData = await response.json();
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to edit source');
      }
  ...
  const handleDelete = async (event) => {
    event.preventDefault();
    try {
      const response = await fetch(`${apiUrl}/master/delete-source/${departmentDetails.id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${Token}`
        }
      ...
      const resData = await response.json();
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to delete source');
      }
        ...
        <div className="content">
          <div className="row">
            <div className="col-md-12">
              {/* Page Header */}
              <PageHeader title="Source" count={totalPages}/>
              {/* /Page Header */}
              ...
              <form onSubmit={handleAdd}>
                ...
                <div className="form-wrap">
                  ...
                  <input
                    ...
                    className="form-control"
                    name='name'
                    placeholder="Eg: Phone Call,Social Media"
                    value={formData.name}
                    required
                    onChange={handleInputChange}
                  />
                </div>
                <p style={{ fontSize: 'small', color: '#7695FF' }}>Note: You can add multiple Source at once by separating them with commas (,)</p>
