...
const JobType = () => {
    ...
    const handleAdd = async (event) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/master/add-job-type`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                throw new Error(resData.message || 'Failed to add job type');
            }
    ...
    const handleEdit = async (event) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/master/edit-job-type`, {
                method: 'PUT',
                headers: {
                    Authorization: `Bearer ${Token}`
                },
            ...
            const resData = await response.json();
            if (!response.ok) {
                throw new Error(resData.message || 'Failed to edit job type');
            }
    ...
    const handleDelete = async (event) => {
        event.preventDefault();
        try {
            const response = await fetch(`${apiUrl}/master/delete-job-type/${jobTypeDetails.id}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${Token}`
                }
            ...
            const resData = await response.json();
            if (!response.ok) {
                throw new Error(resData.message || 'Failed to delete job type');
            }
        ...
        <div>
                ...
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">Add New Job Type</h5>
                            <button
                                className="btn-close"
                            ...
                            <form onSubmit={handleAdd}>
                                ...
                                <div className="form-wrap">
                                    ...
                                    <input
                                        ...
                                        className="form-control"
                                        name='name'
                                        placeholder="Eg: Permanent,Contractual"
                                        value={formData.name}
                                        required
                                        onChange={handleInputChange}
                                    />
                                </div>
                                <p style={{ fontSize: 'small', color: '#7695FF' }}>Note: You can add multiple Job Types at once by separating them with commas (,)</p>
            ...
            <div className="modal custom-modal fade" id="edit_source" role="dialog">
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">Edit Job Type</h5>
                            <button
                                className="btn-close"
                                data-bs-dismiss="modal"
                                aria-label="Close"
