...
const TaskSubCategory = () => {
    ...
    const [tastCategoryOptions, setTastCategoryOptions] = useState([]);
    const [taskSubCategory, setTaskSubCategory] = useState({});

    // console.log('formData =>', formData, num++)
    // console.log('taskSubCategory =>', taskSubCategory, num1++)
    ...
    const fetchDepartmentData = async () => {
        try {
            ...
            setTastCategoryOptions(formattedData);
    ...
    const handleAdd = async (event) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/master/add-task-sub-category`, {
                method: 'POST',
                headers: {
    ...
    const handleEdit = async (event) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/master/edit-task-sub-category`, {
                method: 'PUT',
                headers: {
                    Authorization: `Bearer ${Token}`
                },
            ...
            const resData = await response.json();
            if (!response.ok) {
                throw new Error(resData.message || 'Failed to edit task category');
            }

            setFormData(initialForm)
            fetchData()
            toast.success('Task sub category edited successfully!');
    ...
    const handleDelete = async (event) => {
        event.preventDefault();
        try {
            const response = await fetch(`${apiUrl}/master/delete-task-sub-category/${taskSubCategory.id}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${Token}`
                }
            ...
            setTaskSubCategory({})
            fetchData()
            toast.success('Task sub category deleted successfully!');
                ...
                <div className="content">
                    <div className="row">
                        <div className="col-md-12">
                            {/* Page Header */}
                            <PageHeader title="Task Sub Category" />
                            {/* /Page Header */}
                ...
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">Add New Task Sub Category</h5>
                            <button
                                className="btn-close"
                            ...
                            <form onSubmit={handleAdd}>
                                <div className="form-wrap">
                                    ...
                                    <Select
                                        ...
                                        className="select"
                                        value={tastCategoryOptions.find((ele) => formData.taskCategoryId === ele.value)}
                                        onChange={(event) => {
                                            let { value } = event
                                            handleInputChange({ target: { name: 'taskCategoryId', value } })
                                        }}
                                        options={tastCategoryOptions}
                                    />
                                </div>
                                <div className="form-wrap">
                                    <label className="col-form-label">
                                ...
                                <p style={{ fontSize: 'small', color: '#7695FF' }}>Note: You can add multiple roles at once by separating them with commas (,)</p>
